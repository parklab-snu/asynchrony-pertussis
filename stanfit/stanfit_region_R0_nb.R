...
model <- stan_model("../stanmodel/seir_region_R0_nb.stan")

stanfit_region_R0_nb <- sampling(model,
                           data = standata,
                           seed=103,
                           chain=4,
                           cores=4,
...
save("stanfit_region_R0_nb", file="stanfit_region_R0_nb.rda")
...
plot(standata$cases[,20])
lines(Cmat[,20])

plot(ss$summary[grepl("R0\\[", rownames(ss$summary)),6])
