...
A stochastic version of the SEIR model is implemented based on the Euler-multinomial scheme \citep{king2016statistical}:
\begin{align}
\beta_m (t) &= \mathcal R_{0,m} (1-\exp(-\gamma\Delta t)) \delta(t)\\
\textrm{FOI}_m(t) &= \frac{\beta_m (t) I_m(t-\Delta t)}{N_m}\\
\Delta S_m(t) &\sim \mathrm{Binomial}\left(S_m(t-\Delta t), 1- \exp(-\textrm{FOI}_m(t) \Delta t)\right) \\
...
\end{align}
The observation model is assumed to follow the Poisson distribution: 
\begin{align}
\mathrm{cases}_{t,m} &\sim \mathrm{Poisson}(\rho_m \Delta S_m(t)).
\end{align}
All parameters are defined as in the deterministic model.
...
\caption{
\textbf{Relationship between population size, density, school-aged population fraction and parameter estimates for the basic reproduction number (A,D,G), initial infected fraction (B,E,H), and reporting rate (C,F,I).}
Points represent posterior medians.
Error bars represent 95\% credible intervals.
...
\begin{figure}[!th]
\includegraphics[width=\textwidth]{../figure/figure_compare_supp_2.pdf}
\caption{
\textbf{Comparisons between estimated basic reproduction number and initial susceptible fraction across two different models.}
Each point represents the estimated posterior median for the basic reproduction number $\mathcal R_0$ and initial susceptible fraction $S(0)$ from two separate models in each region.
}
\end{figure}
...
\begin{figure}[!th]
\includegraphics[width=0.9\textwidth]{../figure/figure_R_t_nb_db.pdf}
\caption{
\textbf{Sensitivity analysis of spatial variation in the estimates of effective reproduction number assuming longer serial intervals.}
...
\begin{figure}[!th]
\includegraphics[width=\textwidth]{../figure/figure_stanfit_region_R0_nb.pdf}
\caption{
\textbf{SEIR model fits to spatiotemporal pertussis epidemic data assuming negative binomial observation error.}
(A) Comparisons between the observed and predicted epidemic dynamics across municipalities with more than 400 total cases, ordered by latitude.
(B) The bar plot represents the distribution of R squared values for model fits.
The vertical dashed line represents the median.
(C) Relationship between the estimated and predicted center of gravity.
