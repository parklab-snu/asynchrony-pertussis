...
load("../analysis_synchrony/analysis_synchrony_Rt_nb.rda")

analysis_R_t <- read.csv("../analysis_R_t/analysis_R_t_nb.csv") %>%
  merge(population) %>%
  group_by(region) %>%
  arrange(region, time) %>%
  mutate(
...
analysis_synchrony_Rt_nb[1,]$cbar
analysis_synchrony[1,]$cbar

g3 <- ggplot(analysis_synchrony_Rt_nb) +
  ...
  geom_hline(data=analysis_synchrony_Rt_nb[1,], aes(yintercept=cbar), lty=2) +
...
ggsave("figure_R_t_nb.pdf", gfinal, width=8, height=6)
ggsave("figure_R_t_nb.png", gfinal, width=8, height=6)
