...
analysis_R_t_nb_db <- lapply(split(pertussis_korea_spatial, pertussis_korea_spatial$region), function(x) {
  x2 <- x %>%
    arrange(time)
  
  cases <- x2$cases
  
  cases2 <- exp(predict(gam(cases~s(time), data=x2, family=nb)))
...
write.csv(analysis_R_t_nb_db, file="analysis_R_t_nb_db.csv")
